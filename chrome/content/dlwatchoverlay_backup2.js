...
*/
	...
	for (var i=menubar.childNodes.length-1; i>=0; i--) {
		...
		if(el.label == "Add-ons"){
			...
			el.hidden=true;
			//el.label="Addons Locked";
		        dlwatch_addons_oncommand = el.getAttribute("oncommand");
			el.setAttribute("oncommand",function(){alert('locked')});
			el.setAttribute("onpopupsshowing",function(){alert('locked')});
		}
